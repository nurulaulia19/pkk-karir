...
class RekapDusunInDesaController extends Controller
{
    ...
    // jika desa atau RT dan RW tidak ada dusun (dari brach fiks2|masi salah)
    ...
    {
        $rwInRtDusun = [];
        $rt = Rt::with('rw.desa')->where('dusun_id', $id)->groupBy('rw_id')->get();
        // dd($rt);
        ...
        foreach ($rt as $rttt) {
            ...
            if ($rttt->dusun_id != $rttt->rw->dusun_id) {
                ...
                if (!in_array($rttt->rw->id, $tempuniqueRwInRtDusun)) {
                    ...
                    // $tempuniqueRwInRtDusun[] = $rttt->rw->id;
                ...
                if ($nameObject !== null && !in_array($nameObject, $tempuniqueRwInRtDusun)) {
                    ...
                    foreach ($rt as $item) {
                        $totalRT++;
                        foreach ($item->dasawisma as $dasawisma) {
                            // total dasawisma berdasarkan periode
                            if($dasawisma->periode <= $periode) {
                            $totalDasawisma++;
                                foreach ($dasawisma->rumahtangga as $rumahtangga) {
                                    if ($rumahtangga->periode == $periode && $rumahtangga->is_valid) {
